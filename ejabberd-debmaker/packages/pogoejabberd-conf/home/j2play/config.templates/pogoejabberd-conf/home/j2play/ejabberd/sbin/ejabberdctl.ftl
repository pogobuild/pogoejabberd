...
if [ "$ETCDIR" = "" ] ; then
    ETCDIR=/home/j2play/ejabberd/etc/ejabberd
fi
...
if [ "$LOGS_DIR" = "" ] ; then
    LOGS_DIR=/home/j2play/ejabberd/var/log/ejabberd
fi
if [ "$SPOOLDIR" = "" ] ; then
    SPOOLDIR=/home/j2play/ejabberd/var/lib/ejabberd
fi
if [ "$EJABBERD_DOC_PATH" = "" ] ; then
    EJABBERD_DOC_PATH=/home/j2play/ejabberd/share/doc/ejabberd
fi
...
NAME=-name
[ "$ERLANG_NODE" = "${r"${ERLANG_NODE%.*}"}" ] && NAME=-sname

if [ "$FIREWALL_WINDOW" = "" ] ; then
    KERNEL_OPTS=""
else
    KERNEL_OPTS="-kernel inet_dist_listen_min ${r"${FIREWALL_WINDOW%-*}"} inet_dist_listen_max ${r"${FIREWALL_WINDOW#*-}"}"
fi
...
if [ "$EJABBERDDIR" = "" ]; then
    EJABBERDDIR=/home/j2play/ejabberd/lib/ejabberd
fi
...
ctl ()
{
    ...
    MAXCONNID=100
    CONNLOCKDIR=/home/j2play/ejabberd/var/lock/ejabberdctl
    FLOCK='/usr/bin/flock'
    if [ ! -x "$FLOCK" ] || [ ! -d "$CONNLOCKDIR" ] ; then
        JOT='/usr/bin/jot'
        if [ ! -x "$JOT" ] ; then
            # no flock or jot, simply invoke ctlexec()
            CTL_CONN="ctl-${r"${ERLANG_NODE}"}"
            ctlexec $CTL_CONN $COMMAND
            result=$?
        else
            # no flock, but at least there is jot
            RAND=`jot -r 1 0 $MAXCONNID`
            CTL_CONN="ctl-${r"${RAND}-${ERLANG_NODE}"}"
            ctlexec $CTL_CONN $COMMAND
            result=$?
        fi
    else
