...
cd $WORKSPACE/pogofacebook/debmaker
if [ -e debian-pkg.tar ];then
  rm -rf debian-pkg.tar
fi

# no longer needed as platform no longer builds genius-app's
#$WORKSPACE/pogofacebook/debmaker/scripts/version_profiles.sh "debmaker/data/versions.tdd"

cd $WORKSPACE/pogofacebook/debmaker

tar zcvf debian-pkg.tar * > /dev/null
